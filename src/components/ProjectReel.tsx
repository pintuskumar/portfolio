...
function ReelCard({ project, index }: { project: (typeof projects)[number]; index: number }) {
  return (
    <div className="group relative flex h-[400px] w-[340px] sm:w-[400px] shrink-0 flex-col rounded-2xl border border-white/10 bg-gray-900/80 p-6 backdrop-blur-sm transition-all duration-500 hover:border-indigo-500/30 hover:shadow-xl hover:shadow-indigo-500/5 snap-center">
      {/* Project number */}
...
// Desktop: scroll-jacked horizontal reel
...
// Mobile: native horizontal touch scroll with swipe arrows
function MobileReel() {
    ...
    <div className="py-24 px-6">
      ...
      <div className="relative">
        ...
        >
          ...
          {/* End card */}
          <div className="flex h-[400px] w-[300px] shrink-0 flex-col items-center justify-center rounded-2xl border border-dashed border-white/10 p-6 snap-center">
            <p className="mb-3 text-lg font-semibold text-gray-400">
              Want to see more?
            </p>
            <a
        ...
        {canScrollLeft && (
          <button
            onClick={() => scroll("left")}
            className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-2 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-gray-900/90 border border-white/10 text-white shadow-lg cursor-pointer"
        ...
        {canScrollRight && (
          <button
            ...
            className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-2 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-gray-900/90 border border-white/10 text-white shadow-lg cursor-pointer"
...
export default function ProjectReel() {
  ...
  return (
    <section id="projects" className="relative bg-gray-950">
      {isMobile ? <MobileReel /> : <DesktopReel />}
    </section>
  );
}
